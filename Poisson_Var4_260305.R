...
for(i in 1:num_sim){
  ...
  C.1 <- dataset.1[, ncol(dataset.1) - 1]
  ...
  B.1   <- splineDesign(knots = knots,u.1,ord = qn) 
  ...
  B.h   <- splineDesign(knots = knots,u.h,ord = qn)
...
# save.image(file=paste0("pm poisson ",n,"n var4_260313.RData"))
